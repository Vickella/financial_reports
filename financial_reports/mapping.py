...
def infer_mapping(account) -> Mapping:
	...
	if root == "Asset":
		...
		if type_ in {"Fixed Asset", "Accumulated Depreciation"} or _contains(name, "property", "equipment", "right.of.use"):
		...
		if _contains(name, "prepayment", "advance", "deposit"):
	...
	if root == "Expense":
		if _contains(name, "income tax", "corporate tax"):
		...
		if _contains(name, "selling", "marketing", "advert"):
		...
		if _contains(name, "impairment", "write.down"):
...
def apply_mapping(account, force: bool = False) -> bool:
	...
	values = {
		...
		"custom_ifrs18_mapping_source": "Automatic installation rule",
