...
def execute(filters=None):
	...
	for name in frappe.get_all("IFRS 18 Management Performance Measure", filters={"company": filters.company, "active": 1}, pluck="name"):
		...
		base_row = {
			"measure": measure.measure_name, "item": measure.comparable_subtotal,
			"row_type": _("IFRS subtotal"), "currency": currency, "is_emphasis": 1,
		}
		totals = {}
		for period in periods:
		...
		for adjustment in measure.adjustments:
			row = {
				...
				"account": adjustment.account, "row_type": _("Reconciling item"),
				"currency": currency, "indent": 1,
			}
			for period in periods:
			...
			data.append(row)
		total_row = {
			"measure": measure.measure_name, "item": measure.measure_name,
			"row_type": _("Management-defined performance measure"),
			"reason": measure.reason_for_use, "currency": currency, "is_total": 1,
		}
		for period in periods:
	...
	columns = [
		...
		{"fieldname":"account","label":_("Account"),"fieldtype":"Link","options":"Account","width":190,"print_hide":1},
	]
	for period in periods:
		columns.extend([
	...
	columns.extend([
		{"fieldname":"row_type","label":_("Type"),"fieldtype":"Data","width":170,"print_hide":1},
